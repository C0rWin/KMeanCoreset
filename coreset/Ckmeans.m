function [idx, C, sumD, D] = Ckmeans(X, k,weight, varargin)
...
for rep = 1:reps
    ...
    try % catch empty cluster errors and move on to next rep
        ...
        nonempties = find(m>0);
        D(:,nonempties) = distfun(X, C(nonempties,:),weight, distance, iter);
        d = D((idx-1)*n + (1:n)');
...
while true
    ...
    [C(changed,:), m(changed)] = gcentroids(X, idx,weight, changed, distance);
    D(:,changed) = distfun(X, C(changed,:), weight,distance, iter);
    ...
    if ~isempty(empties)
        ...
        case 'singleton'
            ...
            for i = empties
                ...
                m(i) = 1;
                idx(lonely) = i;
                D(:,i) = distfun(X, C(i,:),weight, distance, iter);

                % Update clusters from which points are taken
                [C(from,:), m(from)] = gcentroids(X, idx,weight, from, distance);
                D(:,from) = distfun(X, C(from,:), weight,distance, iter);
...
function [centroids, counts] = gcentroids(X, index,weight, clusts, dist)
%GCENTROIDS Centroids and counts stratified by group.
[n,p] = size(X);
num = length(clusts);
centroids = NaN(num,p);
counts = zeros(num,1);
sumweight=zeros(num,1);

A=bsxfun(@times,X,weight);
for i = 1:num
    members = (index == clusts(i));
    memweight=members.*weight;
    if any(members)
        counts(i) = sum(members);
        sumweight(i)=sum(memweight);
        switch dist
        case 'sqeuclidean'
            centroids(i,:) = sum(A(members,:),1) / sumweight(i);
        case 'cityblock'
            % Separate out sorted coords for points in i'th cluster,
